# Pacenote visual-telemetry plugin — developer entry points. Every target is what CI runs.
SHELL := /bin/bash
...
.PHONY: help check build vet lint lint-fix fmt fmt-check test test-postgres cover bench bench-smoke tidy-check vuln samples dist clean
...
## bench: what a lap costs to draw
bench:
...
## samples: redraw the README's charts into docs/ from the test fixtures
samples:
	VT_SAMPLE_DIR=$(CURDIR)/docs go test -count=1 -run TestWriteSampleChart .
	rm -f docs/lap.svg

## dist: the folder to drop into the server's plugin directory, as dist/visual-telemetry
dist: build
	rm -rf $(DIST)
	mkdir -p $(DIST)
	go build -trimpath -o $(DIST)/visual-telemetry ./cmd/visual-telemetry
	cp plugin.json $(DIST)/
